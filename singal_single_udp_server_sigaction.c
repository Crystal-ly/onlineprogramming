...
void sig_io_handler(int signo, siginfo_t *SigInfo,void *buff){
//	
...
// 清空buf变量存储空间
	...
	}else if ((SignInfo->si_code)==POLL_OUT){

	}else if ((SignInfo->si_code)==POLL_MSG){

	}else if ((SignInfo->si_code)==POLL_ERR){

	}else if ((SignInfo->si_code)==POLL_PRI){

	}else if ((SignInfo->si_code)==POLL_HUP){
...
//定义信号处理sigaction结构
	...
	act.sa_sigaction=sig_io_handler;
...
//创建UDP套接字
	...
	sigaction(SIGIO, &act,NULL);//注册处理函数
